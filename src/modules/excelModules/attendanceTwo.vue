<template>
  <div class="attendanceTwo">
    ...
    <!-- 确认非正常工作日情况 -->
    <AttendanceTable :attendanceTableData="newArr"></AttendanceTable>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleData () {
      this.yearAndMonth.year = this.jsonData[1].__EMPTY_1.slice(0, 4)
      this.yearAndMonth.month = this.jsonData[1].__EMPTY_1.slice(5, 7)
      ...
      this.newArr = keyArr.map((key, index) => {
        ...
        if (this.attendanceObj[key]) {
          ...
          // 打卡array
          newItem.attendanceArr = this.attendanceObj[key]
            .replace(/(.{5})/g, '$1/')
            .split('/')
            .slice(0, -1)
        } else {
          newItem.attendance = ''
          newItem.attendanceArr = ''
        }
        // 默认上下班时间
        newItem.goToWork = newItem.attendanceArr[0] || ''
        newItem.goOffWork =
          newItem.attendanceArr.length > 1
            ? newItem.attendanceArr[newItem.attendanceArr.length - 1]
            : ''
        // 下班哪天
        if (!newItem.attendance) {
          newItem.selectDay = ''
        } else if (
          Number(newItem.goToWork.slice(0, 2)) <
          Number(newItem.goOffWork.slice(0, 2))
        ) {
          newItem.selectDay = 'today'
        } else {
          newItem.selectDay = 'tomorrow'
        }
        // 备注
        newItem.remark = ''
        return newItem
